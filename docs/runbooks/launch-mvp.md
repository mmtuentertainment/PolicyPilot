...
1. **Provisioning host — the literal 3rd Free project is blocked.** A **third _active_ Free Supabase project in a separate Free org is not available**: the Free two-active-project entitlement is **account-global** across every org where the operator is Owner/Admin (confirmed 2026-06-20 against the current Supabase billing doc), and dev + TEST already occupy both active slots. Choose one operator-approved path before creating the project, cheapest first: **(a)** keep $0 on Supabase by **pausing a non-prod project** first (paused projects don't count against the 2-active cap) — but the paused project goes **offline**, which breaks local `verify:phase-N` against it; **(b)** **Supabase Pro _without_ PITR (paid tier; ~$25/mo approx. as of 2026-06 — confirm current Supabase pricing)** — the clean flip, both non-prod projects stay live; **(c)** a pre-revenue **$0 non-Supabase host** (e.g. Neon Free) — **ASK-FIRST**, because it needs an RLS/session-claim shim and also idle-pauses.
2. **PITR is waived regardless of host.** PITR is explicitly **waived by operator decision** for the pre-revenue production stand-up; this is **not** an ADR-018 hard requirement (ADR-018 is the append-only-acknowledgment invariant — PITR was mistakenly coupled to it). On Supabase the project can sit on **Pro without PITR** (or the paused-non-prod $0 posture per item 1) and later toggle PITR on from the Dashboard with no code, migration, or env-var change.
3. **Idle-pause note (Free / Neon paths only).** On the Free paths — 1(a) paused-non-prod $0 and 1(c) Neon Free — projects pause after about a week of inactivity; on Supabase Pro (1b) idle-pause does not apply. Where it does apply, the existing Railway daily cron (`railway.json` `0 8 * * *` → `/api/cron/reminders`) performs a database-backed org enumeration when it succeeds, so the Railway worker must be live with a matching `CRON_SECRET` to act as the keep-alive.
