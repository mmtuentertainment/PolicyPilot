...
**Prod project not yet provisioned.** `scripts/deploy-config.json` carries a `REPLACE_WITH_PROD_PROJECT_REF` placeholder in the `prod` block; `scripts/with-deploy-creds.ps1` rejects this string explicitly (the `REPLACE_WITH_*` guard) to prevent accidental use against the wrong env. The literal "third _active_ Free Supabase project in a separate Free org" path is **blocked** — the Free 2-active-project entitlement is account-global per Owner/Admin (confirmed 2026-06-20) and dev + TEST already fill both slots; provision prod via one operator-approved path instead: pause a non-prod project for $0 (it goes offline, breaking local verify), Supabase Pro _without_ PITR (paid tier; the clean flip; see `docs/runbooks/launch-mvp.md` §2 for current cost), or an ASK-FIRST $0 non-Supabase host (e.g. Neon Free + RLS shim). PITR is operator-waived for the pre-revenue launch path (not an ADR-018 requirement) and is a later Dashboard toggle on Pro. See `docs/runbooks/launch-mvp.md` §2 (authoritative host-selection) and `.planning/consultant/risk_register.md` R-015. When the prod Supabase project is created, populate the placeholder with the real `project_ref` + store the password via `./scripts/store-deploy-password.ps1 prod` before running any `pnpm db:*:prod` command.
...
- `qa_citation_grants` (Phase 5) columns + wrapped RLS + indexes present
- `organizations` (Phase 6) billing columns + partial-unique indexes present
