...
key-decisions:
  ...
  - "BLOCKING gate (Task 5) executed via DEVIATION from plan-locked command — flagged for operator awareness. PLAN-LOCKED command was `pnpm db:migrate:test` (per Plan 04-02 Task 5 + verify:phase-4 chain expectations). ACTUAL command used: exported DATABASE_URL=$DATABASE_URL_TEST + DIRECT_URL=$DIRECT_URL_TEST from .env.local and invoked drizzle-kit migrate directly (mirrors scripts/check-data-layer.ts pattern). REASON: `pnpm db:migrate:test` uses --env-file=.env.local.test which points at the intentionally-empty placeholder file from the SF-DB-1 deferral (Phase 2 Plan 02-03 SUMMARY); per Plan 02-06 SUMMARY the orchestrator's `checkMigrateTest` already overrides DATABASE_URL + DIRECT_URL from `_TEST` env vars via spawnSync rather than relying on a second env file. STATUS: deviation accepted by execution path; operator approval requested for canonical-vs-workaround reconciliation (either populate .env.local.test or amend the plan's locked command to the override pattern). Decision tracking: tied to SF-DB-1 deferral chain; no decision ID assigned yet; approval timestamp pending operator sign-off during /gsd-ship review."
