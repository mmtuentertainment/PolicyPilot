...
- **Self-hosted open-source model migration** (e.g., Qwen2.5-7B / Granite-3.1-8B / Hermes-3 + XGrammar on consumer GPU) — deliberate post-MVP optimization, NOT Phase 4 scope. Rationale: documented self-hosting breakeven is ~100M tokens/mo on 70B-class hardware; projected 100-org workload is ~5–10M tokens/mo (2 orders of magnitude below breakeven), and multi-tenant inference on 8 GB VRAM serializes to ~1–2 concurrent sessions before queue degrades UX. The SLM-agentic literature (NVIDIA Belcak 2025 [arxiv:2506.02153]; Sharma–Mehta 2025 [arxiv:2510.03847]) supports SLMs only on schema-constrained narrow tasks — Consistency Check is the one Phase 4 surface where the win condition is documented; Draft + Q&A have no published benchmark showing 7–9B models matching Sonnet 4.6 on long-form professional writing. Re-evaluate as a Phase 8 / 9 cost-optimization spike with explicit migration criterion: per-surface schema-validity + output-quality match ≥ 90% vs Sonnet on a real fixture set. (Research basis: `/gsd-spec-phase 4` session 2026-05-21; full findings in branch commit log.)
