...
- **D-07 — Route `app/api/reports/acknowledgments/route.ts`, `GET(req): Promise<Response>`.** Auth + business both run INSIDE a single in-route `try` whose `catch` maps typed errors to status codes — because a Next.js Route Handler has NO error boundary (an uncaught throw = HTTP 500, verified in Next 15.5 `app-route.ts`). Shape: `try { const ctx = await getOrgContext(); requireAdminFromCtx(ctx); /* param parse (D-08) — zod failure short-circuits to 400 */ const rows = await withOrgScope(ctx, (s) => Reports.listAckComplianceForOrg(s, filters)); const enriched = await enrichWithClerkIdentity(rows); /* serialize per format */ } catch (err) { if (err instanceof ForbiddenError) return 403 {error:'forbidden'}; if (err instanceof BootstrapError) return 401; /* ForbiddenError checked FIRST — it extends BootstrapError, errors.ts:223 */ if (err instanceof ZodError) return 400 {error:'invalid_request'}; /* masked console.error (D-36) */ return 503 {error:'service_unavailable'}; }`. Capture `ctx.orgId` for the 503 log where available (declare `let ctx` before the try, or log without orgId when ctx is unresolved). Reference idiom: `app/api/ai/consistency/route.ts:113-144` discriminates `TierLimitExceededError → err.statusCode` the same in-route way.
- **D-08 — Param validation via zod over `searchParams`.** `const sp = new URL(req.url).searchParams;` parse `{ format: z.enum(['json','csv']).default('json'), policyId: z.string().uuid().optional(), departmentId: z.string().uuid().optional() }` with `safeParse`. On failure → `NextResponse.json({ error: 'invalid_request' }, { status: 400 })` (an unknown `format` like `xml` fails the enum → 400; a malformed UUID fails → 400). Param parsing/validation runs INSIDE the single in-route `try`, after the auth calls and before `withOrgScope`; a `safeParse` failure short-circuits to 400 (or its `ZodError` is caught by the same catch and mapped to 400).
- **D-09 — Hand-rolled CSV serializer in NEW `lib/reports/csv.ts` (dependency-free).** Generic `toCsv(headers: string[], rows: string[][]): string` + a `csvField(value)` escaper. RFC-4180: a field containing `,`, `"`, CR, or LF is wrapped in double quotes with internal `"` doubled; records terminated with CRLF (`\r\n`); UTF-8. **Formula/CSV-injection guard:** if a field's first NON-WHITESPACE character ∈ {`=`, `+`, `-`, `@`, TAB(`\t`), CR(`\r`)} prefix a single-quote (`'`) at the start of the field BEFORE quoting — applied to ALL cells (cheap and uniformly safe; user-controlled fields = policy title, employee name, email, department name are the live vectors). A leading space/tab before the trigger does NOT bypass the guard (a spreadsheet trims leading whitespace before formula evaluation, so ` =cmd` is still neutralized to `' =cmd`). **Cell coercion (serializer-owned):** `csvField` accepts `string | number | null | undefined`; it coerces `null`/`undefined` → `''` and `number` → `String(value)` BEFORE guarding/quoting, so a Date-formatted/numeric/`null` cell from the route never throws and never emits the literal `'null'`. Column contract (fixed order) finalized in PLAN; baseline: `Employee Name, Email, Department, Policy Title, Status, Acknowledged At, IP Address, Assigned At, Policy Version`. `ackState` rendered as a human label per D-04.
...
- **D-11 — Error shapes + logging.** All status mapping is IN-ROUTE inside the single try/catch (Route Handlers have NO error boundary — an uncaught throw = HTTP 500): 403 `{error:'forbidden'}` (catch `ForbiddenError` FIRST — it extends BootstrapError); 401 (catch the unauth `BootstrapError` set — NotAuthenticatedError/NoActiveOrganizationError from getOrgContext); 400 `{error:'invalid_request'}` (catch `ZodError` from safeParse); 503 `{error:'service_unavailable'}` (DB/Clerk failure, the else branch) with a PII-safe structured `console.error` (D-36 masked-logging pattern: `{ orgId: ctx?.orgId, error: {name, message: msg.slice(0,120)} }`). No `any` anywhere.

### Testing & Verification (R8-2, R8-3, R8-7)
- **D-12 — Three-layer test strategy.** (a) **Route unit** `app/api/reports/acknowledgments/route.test.ts` — mock `getOrgContext` (admin / employee / unauth via REJECT with NotAuthenticatedError), use the REAL (or throw-mocked) `requireAdminFromCtx` so it actually throws ForbiddenError, mock `Reports.listAckComplianceForOrg` → fixture rows, mock `enrichWithClerkIdentity` (Clerk module boundary); EVERY case asserts `res.status` + `await res.json()` (NEVER `rejects.toThrow`): 401 / 403 `{error:'forbidden'}` / 400 `{error:'invalid_request'}` / 200; JSON shape + summary math; CSV `Content-Type` + `Content-Disposition` + dated filename + first byte is the UTF-8 BOM. (b) **Serializer unit** `lib/reports/csv.test.ts` — RFC-4180 cases (`",` round-trip; embedded newline stays in one quoted field; quote-doubling) + formula-injection (`=cmd` → `'=cmd`, `+`, `-`, `@`, leading TAB) + the guard-then-quote COMBINATION (`=a,b` → `"'=a,b"`) + leading-whitespace guard (` =cmd` → `' =cmd`) + null/number coercion (`null`→`''`, `3`→`'3'`) + UTF-8 BOM prefix + header-row exactness. (c) **TEST-DB integration** `scripts/check-reports.test.ts` (vitest) + `scripts/check-reports.vitest.config.ts` — mirrors the REAL gate `scripts/check-employee-portal.test.ts` + `scripts/check-employee-portal.vitest.config.ts` (NOT a bare-tsx `.ts`). BYPASSRLS postgres-js connection seeds inside `sql.begin`; `vi.mock('@/lib/db/scoped')` binds `withOrgScope` to the outer rollback `tx`; the REAL `Reports.listAckComplianceForOrg` is then driven AFTER `SET LOCAL ROLE authenticated` + `set_config('request.jwt.claims', <Org-A claims>, true)` so RLS actually fires (per check-rls.ts:4-14 — a bare BYPASSRLS connection never fires RLS). Includes a required POSITIVE CONTROL (non-empty Org-A result) + an RLS-firing NEGATIVE (zero Org-B rows; Org-B `policyId` under Org A → 0 rows); intentional `__INTENTIONAL_ROLLBACK__` + TRUNCATE cleanup. Proves R8-2 ack-state derivation (none/current/stale) + department fan-out + draft-exclusion + dedup (exactly one row for a dual-assigned user), and R8-3 two-org isolation. Clerk enrichment is stubbed (DB-truth only; no live Clerk in CI).
- **D-13 — Cumulative `verify:phase-8` + CI mirror.** `package.json`: `verify:phase-8 = pnpm tsc --noEmit && pnpm verify:phase-7 && pnpm check:reports && pnpm run test -- --run app/api/reports && pnpm run test -- --run lib/reports && pnpm db:verify && pnpm check:artifacts`. `check:reports` = `tsx --env-file=.env.local node_modules/vitest/vitest.mjs run scripts/check-reports.test.ts --config scripts/check-reports.vitest.config.ts` — the EXACT vitest-runner form `check:crons-email` uses (package.json:45), which passes `--env-file=.env.local` (so `DATABASE_URL_TEST` is set) and the gate hard-fails (never skips) on missing TEST_DATABASE_URL/DATABASE_URL_TEST. NOT bare `tsx scripts/check-reports.ts`. Extend `scripts/check-artifacts.ts` to assert the route + `lib/db/repositories/reports.ts` + `lib/reports/csv.ts` + `lib/reports/enrich.ts` + the three test files exist. New `.github/workflows/verify-phase-8.yml` mirrors `verify-phase-7.yml` (concurrency group, 45-min timeout, `push` on `main` only + `pull_request` + `workflow_dispatch`; same env/secrets placeholders). **Never weaker than `verify:phase-7`** — it runs it in full first.
...
- `scripts/check-reports.test.ts` fixture builders + exact `verify:phase-8` / `check-artifacts` wiring.
...
- Reference route shape: `app/api/ai/consistency/route.ts:59-145` (in-route try/catch discriminating typed errors → status, `withOrgScope`-inside, `NextResponse.json`, masked logging).
...
- `scripts/check-employee-portal.test.ts` + `scripts/check-employee-portal.vitest.config.ts` — the REAL vitest TEST-DB integration gate `scripts/check-reports.test.ts` + `scripts/check-reports.vitest.config.ts` mirror (seed via BYPASSRLS, assert via SET LOCAL ROLE authenticated + set_config, drive the real repository fn through a mocked withOrgScope). `scripts/check-rls.ts:4-14,158-214` — reference ONLY for the seed-vs-assert RLS rationale (it asserts via raw tx.unsafe SQL, never drives a repository function).
...
- **`requireAdminFromCtx`** (`lib/auth/require-admin.ts:56-60`) — API-route admin gate → throws `ForbiddenError`; called INSIDE the in-route try and caught → 403 (checked before the unauth BootstrapError→401 branch).
...
- **Module-boundary mocking** (06 D-32) — Clerk enrichment + repo mocked in route unit tests; live DB only in `check-reports.test.ts`.
...
- **In-route catch-and-map** (D-07): Route Handlers have no error boundary, so auth + business run inside a single `try` and the `catch` discriminates typed errors to status codes — the reference `app/api/ai/consistency/route.ts` discriminates `TierLimitExceededError → err.statusCode` the same in-route way.
...
- `scripts/check-reports.test.ts` + `scripts/check-reports.vitest.config.ts` — TEST-DB integration gate (vitest).
