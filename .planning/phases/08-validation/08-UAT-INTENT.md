...
7. **CSV safety.** A policy titled with a leading `=`/`+`/`-`/`@` (or containing commas/quotes/newlines) exports without breaking the CSV and without the spreadsheet executing it as a formula (neutralized with a leading `'`).
   - **Leading-whitespace bypass.** A title with whitespace *before* a dangerous prefix (e.g. `" =danger"`, `"\t+danger"`) is still neutralized — the guard inspects the first non-whitespace character (`value.trimStart()[0]`), so the `'` prefix is prepended ahead of the original leading space/tab (e.g. `" =danger"` → `"' =danger"`). A leading TAB/CR by itself also triggers the guard. Whitespace alone cannot smuggle a formula past the guard.
   - **Null and numeric (non-string) cells.** Fields that aren't strings export safely without throwing: a `null`/`undefined` cell (e.g. an unacknowledged row's `Acknowledged At` / `IP Address`) renders as an empty cell, and a numeric cell (e.g. `Policy Version` = `3`) renders as its string form (`3`). Non-string values are coerced before escaping, so the export never breaks on missing or numeric data.
