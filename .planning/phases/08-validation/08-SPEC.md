...
- **Auth + org-scope seams are established:** `getOrgContext()` (`lib/auth/context.ts:93-174`, Clerk text-id → internal UUID) THROWS typed errors (`NotAuthenticatedError`/`NoActiveOrganizationError` for the unauth case, all `instanceof BootstrapError`); `requireAdminFromCtx(ctx)` (`lib/auth/require-admin.ts:56-60`) THROWS `ForbiddenError` (also `instanceof BootstrapError`) on non-admin — distinct from the page-level `requireAdmin()` which `notFound()`s. NOTE: Route Handlers have no error boundary (an uncaught throw = HTTP 500), so the route must CATCH these and map `ForbiddenError → 403 {error:'forbidden'}` (checked first), the unauth `BootstrapError` set → 401, else → 503; `withOrgScope(ctx, fn)` (`lib/db/scoped.ts:41-67`) opens an RLS-enforced tx (`SET LOCAL ROLE authenticated` + `set_config('request.jwt.claims', …, true)`) and yields `OrgScope = OrgContext & { tx }`. Canonical route shape: auth + business BOTH run INSIDE a single in-route `try` whose `catch` maps the typed errors above (ForbiddenError→403 first, unauth BootstrapError→401, ZodError→400, else→503), `NextResponse.json(...)`. Reference route: `app/api/ai/consistency/route.ts:59-145`.
...
1. **R8-1 Admin-only authenticated report endpoint**: `GET /api/reports/acknowledgments` authenticates via Clerk (`getOrgContext()`) and authorizes admin-only via `requireAdminFromCtx(ctx)`.
   ...
   - Target: route handler present; unauthenticated → **401**; authenticated non-admin (employee/reviewer) → **403 `{ error: 'forbidden' }`**; admin → 200. Status mapping is done IN-ROUTE: a Next.js Route Handler has NO error boundary — an uncaught throw returns HTTP 500 (verified in Next 15.5 framework source `app-route.ts`). So the route runs `getOrgContext()` + `requireAdminFromCtx(ctx)` and CATCHES their typed errors, discriminating `ForbiddenError` (extends BootstrapError) → 403 `{ error: 'forbidden' }` FIRST, then unauth `BootstrapError` (NotAuthenticatedError / NoActiveOrganizationError) → 401, letting `ClerkAuthFailedError` + all else fall through to 503. (Mirrors how the shipped AI routes route `TierLimitExceededError → err.statusCode` in-route; `app/api/ai/consistency/route.ts:64-65,113-144`.) No `any`.
...
3. **R8-3 Cross-org isolation (RLS-enforced)**: the report never returns another org's rows under any param.
   ...
   - Target: isolation enforced by RLS inside `withOrgScope` (the route never queries raw `db`); a two-org TEST-DB integration test (mirroring the REAL vitest gate `scripts/check-employee-portal.test.ts` + `scripts/check-employee-portal.vitest.config.ts`; `check-rls.ts:4-14,158-214` for the seed-vs-assert RLS rationale only) proves Org A's export contains zero Org B rows even when both orgs share policy titles and a `policyId`/`departmentId` filter is supplied.
...
5. **R8-5 Hand-rolled RFC-4180 CSV serializer with formula-injection neutralization**: `format=csv` returns a correctly-escaped CSV; no CSV/spreadsheet package added.
   ...
   - Target: a dependency-free serializer with a stable header row + fixed column order; fields containing `,` `"` CR or LF are wrapped in double-quotes with internal `"` doubled; CRLF (`\r\n`) record terminators; UTF-8. **CSV/formula-injection guard:** `csvField` first coerces `null`/`undefined`→`''` and `number`→`String(value)`, then any field whose first NON-WHITESPACE character is `= + - @`, TAB, or CR is prefixed with a `'` (apostrophe) guard before quoting (a leading space/tab does NOT bypass the guard), applied to ALL cells. The whole document is prefixed once with a UTF-8 BOM (U+FEFF) by `toCsv` so Excel-on-Windows renders non-ASCII correctly.
...
7. **R8-7 Cumulative `verify:phase-8` gate + CI workflow**: a new `verify:phase-8` script chains `verify:phase-7` and adds Phase 8 checks; a `verify-phase-8.yml` mirrors `verify-phase-7.yml`.
   ...
   - Target: `verify:phase-8 = pnpm tsc --noEmit && pnpm verify:phase-7 && pnpm run test -- --run app/api/reports && pnpm run test -- --run lib/reports && pnpm check:reports && pnpm db:verify && pnpm check:artifacts` (exact sub-checks finalized in PLAN; **never weaker than `verify:phase-7`**). A `check:artifacts` extension asserts the route + repo + serializer + tests exist (the new Phase-8 checks hand-spread into check-artifacts.ts main()'s `all: Check[]` array); `check:reports` is the vitest TEST-DB integration gate that drives the real query through RLS. `verify-phase-8.yml` mirrors `verify-phase-7.yml`'s concurrency-group + 45-min timeout + push-on-`main`-only + `workflow_dispatch` (TRUNCATE-deadlock flake guard).
...
- Tests: co-located vitest (route + serializer unit, Clerk enrichment mocked, auth/403/400 paths) **and** a TEST-DB integration test proving cross-org isolation + ack-state derivation (mirrors the REAL vitest gate `scripts/check-employee-portal.test.ts` + `.vitest.config.ts`; `check-rls.ts` for the RLS seed-vs-assert rationale only).
...
- **Admin-only:** `requireAdminFromCtx(ctx)` (API-route variant) runs INSIDE the in-route `try`; its `ForbiddenError` is caught and mapped to 403 `{ error: 'forbidden' }` (checked FIRST, before the unauth BootstrapError→401 branch, since ForbiddenError extends BootstrapError).
...
- **CSV safety:** RFC-4180 escaping + UTF-8 BOM + formula/CSV-injection neutralization (first NON-WHITESPACE char guard) applied to ALL cells after `null`/`number` coercion.
