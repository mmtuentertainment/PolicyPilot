...
| 10 | `verify:phase-1` + `check:db` scripts wired in package.json | **PASS** | `package.json:15-16` — `verify:phase-1: tsx --env-file=.env.local scripts/check-foundation.ts && pnpm check:artifacts` (chained per VALIDATION.md gap-fill), `check:db: tsx --conditions=react-server --env-file=.env.local scripts/check-db.ts`. |
