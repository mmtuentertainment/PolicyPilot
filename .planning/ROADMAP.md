...
**Plans**: 14 plans
- [ ] 04-01-PLAN.md — Wave 0: SDK install + .env.local.example + scratch/probe.ts D-39 audit + PROMPTS.md/API-SPEC.md/SCHEMA.md amendments (D-01/10/27/29/31/35)
- [ ] 04-02-PLAN.md — Wave 0: Drizzle schema + migrations 0005/0006/0007 + BLOCKING pnpm db:migrate:test gate (D-06/29/32/34/35)
- [ ] 04-03-PLAN.md — Wave 0: 14 RED test stubs + tests/ai-mocks.ts shared fixtures helper (covers AC-23..AC-33 + SP-1..SP-4)
- [ ] 04-04-PLAN.md — Wave 1: lib/ai foundation (client/models/cache/prompts/extract/schemas) — D-02/03/04/33/38/42; AC-28 + AC-33 GREEN
- [ ] 04-05-PLAN.md — Wave 1: lib/ai Q&A helpers (qa-extract/qa-parser) + tests/types.ts D-43 citation-shape compile-time guard
- [ ] 04-06-PLAN.md — Wave 1: lib/stripe/products.ts + errors.ts + check-error-discipline.ts widening (D-14/15/16)
- [ ] 04-07-PLAN.md — Wave 1: repositories (ai_generations fill + policies extend + batch_jobs new) + lib/auth/errors.ts ForbiddenError + require-admin.ts D-45 amendment
- [ ] 04-08-PLAN.md — Wave 2: lib/ai/summary.ts + POST /api/ai/draft + POST /api/ai/summary (SPEC R2/R3 + D-19/32/35/36/37)
- [ ] 04-09-PLAN.md — Wave 2: POST /api/ai/qa (SPEC R4 + D-33c/40/41/46 — no tier check, same-closure validIds, LONG_CACHE ordering)
- [ ] 04-10-PLAN.md — Wave 2: POST /api/ai/consistency + GET /api/ai/consistency/[batchId] + SDK→SPEC translator + check-rls.ts batch_jobs extension (SPEC R5 + AC-24/30 + D-34)
- [ ] 04-11-PLAN.md — Wave 2: publish() post-commit summary hook (D-19) + scripts/check-ai-prompts.ts ts-morph anchor gate (D-26)
- [ ] 04-12-PLAN.md — Wave 3: PolicyAiDraftDialog + PolicyRegenerateTldrButton + wire into PolicyEditor / PolicyView (D-22/28 + AC-23)
- [ ] 04-13-PLAN.md — Wave 3: /dashboard/consistency page + ConsistencyCheckRunner + 4 admin components + AdminSidebar entry (D-20/21/23/30 + AC-25)
- [ ] 04-14-PLAN.md — Wave 4: scripts/check-ai-layer.ts integration test + check-artifacts.ts Phase 4 scaffold + verify:phase-4 chain (D-24) + operator UAT checkpoint
