...
| Notification records + in-app bell | 5-7 | Hardening / in-PR (PR #44, not merged) | System/employee | Indirect | Medium | Medium | Phase 7 implements `Notifications.create()`/`markRead()` (were throw-stubs) plus `markAllReadForUser` server action and the `NotificationBell`/`NotificationBellServer` UI (unread count from `notifications.read=false`; mark-as-read decrements without full reload). `notifications` stays intentionally mutable (NOT in IMMUTABLE_TABLES). Executed on `gsd/phase-7-crons-email`, published as draft PR #44 (tip `9a3ebe2`); verify:phase-7 green (first green at `5d304b4`, re-verified after FU folds at `aa6d8ab`); not yet merged. |
...
1. Phase 7 reminders — completes the acknowledgment follow-up loop; built + verify-green (first green at `5d304b4`, re-verified at `aa6d8ab`), published as draft PR #44 (not merged). Next leverage is operator un-draft/merge (after reconciling the environmental CI red) + the operator-executed live Railway run.
