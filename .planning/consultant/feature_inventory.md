...
Updated: 2026-06-16 - HEAD-pin `7ba6ba2` (`main`). **Phase 8 Validation (CSV-first slice, AC#5) SHIPPED to `main` via PR #48 squash commit `03c18d4` (2026-06-16) — 8th/FINAL phase; v1.0 build COMPLETE.** CSV export flips In progress -> Shipped / monitor (`03c18d4`); compliance dashboard donut / recharts stays Deferred (ASK-FIRST + ≥14-day rule); full acceptance-test pass stays Pending (Stripe test-clock AC#6 + beat-manual SC#5 + seed harness deferred). Prior: 2026-06-15 - Phase 7 (Crons + Email) SHIPPED to `main` via PR #44 squash commit `8b7019d` (2026-06-14) — 7th of 8 phases; branch deleted post-merge; main `3df5223`. All Phase 7 features flipped Hardening/in-PR -> Shipped / monitor. The hosted-CI red was environmental and is RESOLVED (merged with gates green). R-006 shipped, R-007 partial+benchmark-deferred, R-018 shipped-but-open. Prior: 2026-06-14 - Phase 7 (Crons + Email) published as draft PR #44 from `gsd/phase-7-crons-email` (tip `9a3ebe2`); `verify:phase-7` green (tsc 0 / 39 vitest files·332 tests / check:rls / db:verify); ship-review `wf_0fa4b84e-ad3` = ship / 0 must-fix + 4 follow-ups (FU-2 folded `6fd033a`, FU-4 folded `aa6d8ab`, FU-1 false-positive, FU-3 hosted CI red = environmental); NOT merged, no deploy, no staging/prod migration, no live email send. Prior: 2026-06-05 - Phase 9 Reviewer / approval-workflow MVP shipped via PR #42 at `1122da5`; prior: Phase 6 shipped via PR #32
...
| CSV export | 8 | Shipped / monitor | Admin/auditor | Direct | High | Low | Small feature, high perceived value. **Phase 8 CSV-first slice (AC#5) SHIPPED to `main` via PR #48 `03c18d4`** (2026-06-16): `GET /api/reports/acknowledgments?format=json\|csv` + org-scoped ack report query (`lib/db/repositories/reports.ts`, MIN(assignedAt)+GROUP BY dedup, `org_id`+RLS on every join) + hand-rolled formula-guarded CSV serializer (leading-whitespace + null/number safe) + tests + `verify:phase-8` green. Ship-review `wf_f53707fe-2a9` = GO / 0 critical / 0 important. |
| Full acceptance-test pass | 8 | Pending | Operator/system | Defensive | High | Medium | Required before confident launch/demo. The CSV-first slice (AC#5) shipped; the full validation-gate pass is DEFERRED to the evidence-capture leg: Stripe test-clock renewal run (AC#6 — backlog rank-24), beat-manual benchmark (SC#5 — rank-25), populated-org seed harness (rank-26). Evidence capture for criteria #1-#4/#7/#8 also deferred per the CSV-first authorization. |
...
2. Phase 8 CSV export/reporting — produces buyer-visible audit evidence; the CSV-first slice SHIPPED to `main` via PR #48 `03c18d4` (2026-06-16). Next leverage is the deferred beat-manual benchmark (SC#5, rank-25) and the compliance dashboard donut (rank-23, ASK-FIRST + ≥14d) before launch/demo.
