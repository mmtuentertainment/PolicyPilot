...
Updated: 2026-06-04 - approvalWorkflows tier-gate RESOLVED → parked (rank 16; risk R-017 Accepted/Parked) + Reviewer-role/review-queue feature filed (rank 17, Phase 9 candidate); stale dead-branch guard retired (`gsd/phase-6-billing` deleted)
...
| 16 | approvalWorkflows tier gate unimplemented — the Growth+ `TIER_LIMITS.approvalWorkflows` flag is enforced nowhere; `requireTierLimit` is called only in `app/api/ai/draft/route.ts:61` + `app/api/ai/consistency/route.ts:72`, never in `lib/policies/`, so the 7 transition orchestrators are not tier-gated. | 6+ | 3 | 2 | 4 | 1 | 5 | 2 | 13 | Parked — see R-017 | **RESOLVED/PARKED 2026-06-04 (operator product-intent): pre-declared future flag, NOT gate-now. Gating `approve()` is cosmetic — `approve()` aliases `publish()` (`transitions.ts:133-135`), so a direct publish bypasses it (publish-leak).** Enforcement folds into the Reviewer feature (rank 17) as a completeness check inside `publish()`. Proposal `ops/proposals/2026-06-04-approvalworkflows-tier-gate.md` → RESOLVED. See R-017 + delta `ops/deltas/2026-06-04-approvalworkflows-intent-resolved.md`. |
| 17 | Build Reviewer-role + review-queue feature (the proper home of `approvalWorkflows` enforcement). Substrate is largely PRE-BUILT: `Role='reviewer'` exists (`lib/auth/context.ts:35`), the `workflow_stages` table is live (`lib/db/schema.ts:328-339`), and dead reviewer-queue repo seams exist (`listPendingForReviewer`/`recordDecision`, `lib/db/repositories/workflow_stages.ts`). Needs a reviewer route group + post-sign-in branch, a `requireReviewer` guard, a completeness-gated `publish()` for Growth+, and the 5 blockers resolved (org_id-per-table, close the publish-leak, no `approved` state, no DB append-only, "drop `workflow_stages`" is destructive+self-defeating). | 9 | 4 | 2 | 3 | 2 | 3 | 5 | 9 | Pending / ASK-FIRST (Phase 9 candidate) | Depends 2/3/6 (email/SLA-reminder slice also needs unstarted Phase 7). Validated read-only in session-11 `wf_c7f9d7cd-de9` (full file:line evidence). Closes the R-017 marketing-vs-product gap. NO build until operator GO + a Codex-vs-Claude routing pick; new tables/RLS/role-gating/tier-gating = ASK-FIRST/security → default Codex. |
...
4. `approvalWorkflows` is PARKED (R-017 Accepted/Parked) pending the Reviewer feature (rank 17, Phase 9 candidate). **OPEN marketing-copy decision (operator):** the pricing page (`app/(marketing)/pricing/page.tsx`) markets "Approval workflows" on the highlighted Growth tile while the feature is unbuilt/unenforced — accept as a roadmap promise vs footnote/soften is the operator's call (deferred, no code touched here). Do not start the Reviewer build (new tables / RLS / role-gating = ASK-FIRST/security) without explicit operator GO + a Codex-vs-Claude routing pick.
