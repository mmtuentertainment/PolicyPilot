...
| 4 | Implement Stripe Checkout + 5-event idempotent webhook. | 6 | 5 | 4 | 5 | 3 | 3 | 4 | 16 | Hardening - 06-06 verifier wired | Foundation, webhook, tier predicate, checkout/pricing intent, Customer Portal/settings, and verifier/UAT checklist are complete locally. Next smallest slice is operator-controlled: apply/verify `0012_billing_state` on the configured `db:verify` target and run masked Stripe sandbox/test-clock UAT. |
...
1. Plans 06-01 through 06-06 verifier wiring are complete locally; Phase 6 is still not shipped.
2. Next smallest slice is applying/verifying existing migration `0012_billing_state` on the configured `db:verify` target, then running Stripe sandbox/test-clock UAT with masked evidence.
