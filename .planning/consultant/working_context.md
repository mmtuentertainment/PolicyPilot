...
Updated: 2026-06-03 - Cause-B lazy-db fix in flight (fix/db-lazy-init); production deploy is Tier-B / operator-gated
...
- Branch topology is now reconciled after ship: the remote PR branch was deleted by merge, product/security scoped diffs between `gsd/phase-6-stripe-uat-complete` and `gsd/phase-6-billing` were empty before merge, and local `gsd/phase-6-billing` should be retired only after Matthew approves branch deletion.
- Cause-B build-time DB coupling fix is in flight on `fix/db-lazy-init` (branched off `origin/main`, NOT the fallow branch): `lib/db/index.ts` is now lazy (side-effect-free import via a Proxy that defers the `DATABASE_URL` check + Postgres client to first runtime access), so `next build`'s "Collecting page data" phase no longer crashes when `DATABASE_URL` is absent. Adds `lib/db/index.test.ts` regression guard + a narrow ADR-023 allow-list entry for that test; full `verify:phase-6` is green (incl. live integration + `db:verify`). Tier B (a working prod deploy: prod Supabase + Vercel runtime env/secrets + staged migrations) remains operator-gated and unstarted — production has never successfully deployed (prod 404; CLI deploys frozen at `bae9174`). No secrets/env touched.
...
- Production has NEVER successfully deployed to Vercel (prod 404 `DEPLOYMENT_NOT_FOUND`; CLI deploys frozen at `bae9174`). The `fix/db-lazy-init` lazy-db change unblocks the build-crash class but is necessary-but-not-sufficient; a real prod deploy is Tier-B / operator-gated (risk R-015).
