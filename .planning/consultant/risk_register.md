...
| R-004 | Stripe billing implementation misses renewal/failure/cancel/update events or idempotency. | Revenue/Ops | 3 | 5 | 15 | Controlled/Active | Plans 06-01 and 06-02 are complete locally: foundation schema/helpers plus 5-event webhook, raw-body verification, canonical Subscription re-fetch, transaction-scoped `stripe_events` idempotency, and M2 HTTP-status matrix. Score stays 15 until checkout/portal/tier gates and Stripe test-clock UAT prove the full billing loop (ROADMAP SC#3 / section 10 #6). |
