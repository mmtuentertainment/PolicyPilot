...
- Root cause history: 
  ...
  - **Cause-B (Prod Supabase project not yet provisioned, THEN pooler auth)**: The blocker is broader than a stale password. (1) The production Supabase project does **not yet exist** — `scripts/deploy-config.json:12` still holds the `REPLACE_WITH_PROD_PROJECT_REF` placeholder. The literal "third _active_ Free project in a separate Free org" path is **blocked** — the Free two-active-project entitlement is account-global per Owner/Admin (confirmed 2026-06-20) and dev + TEST fill both slots; the operator-approved paths are pausing a non-prod project for $0 (it goes offline), Supabase Pro _without_ PITR (paid tier; the clean flip; see `docs/runbooks/launch-mvp.md` §2 for current cost), or an ASK-FIRST $0 non-Supabase host (e.g. Neon Free + RLS shim). PITR is operator-waived for the pre-revenue launch path (not an ADR-018 requirement) and is a later Dashboard toggle on Pro. See `docs/runbooks/launch-mvp.md` §2 and `.planning/consultant/risk_register.md` R-015. (2) Only *after* the prod project is provisioned can the operator configure/rotate the Transaction-pooler (port 6543) password and set `DATABASE_URL`; until both are done, `deploy:preflight` fails authentication against the pooler and the build exits 1.
...
- Action path: 
  1. Operator provisions the production database via an operator-approved path from `docs/runbooks/launch-mvp.md` §2 — pause a non-prod project for $0 (it goes offline, breaking local verify), Supabase Pro _without_ PITR (paid tier, clean flip; both non-prod projects stay live), or an ASK-FIRST $0 non-Supabase host (Neon Free + RLS shim; idle-pauses); the literal 3rd-active-Free-in-separate-org is blocked by the account-global 2-active cap (PITR waived; not an ADR-018 req) — then replaces `REPLACE_WITH_PROD_PROJECT_REF` in `scripts/deploy-config.json`.
