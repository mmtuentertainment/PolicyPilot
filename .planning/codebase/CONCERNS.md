...
- Action path: 
  1. Operator provisions the production database via an operator-approved path from `docs/runbooks/launch-mvp.md` §2 — Supabase Pro _without_ PITR (~$25/mo, clean flip), a paused-non-prod $0 Supabase posture, or an ASK-FIRST $0 non-Supabase host (Neon Free + RLS shim); the literal 3rd-active-Free-in-separate-org is blocked by the account-global 2-active cap (PITR waived; not an ADR-018 req) — then replaces `REPLACE_WITH_PROD_PROJECT_REF` in `scripts/deploy-config.json`.
